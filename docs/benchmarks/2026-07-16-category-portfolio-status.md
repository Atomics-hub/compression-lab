...
| Tabular CSV | Bounded development probe | ✅ TBL1 selector beat direct zstd-19 by 24.02% on four slices; full-corpus Brotli result pending | ❌ Python column path only 0.65-2.69 MB/s encode; native path required | — candidate memory unmeasured | ❌ No | Implement native TBL1, then run clean full-development candidate and frontier baselines |
...
`docs/benchmarks/2026-07-16-tabular-baseline-census.md`. Public-validation
