#!/usr/bin/env python3
"""Verify and atomically import the immutable CLUE-LDS JLS2 first score."""
...
EXPECTED_RUN_ID = 29606109504
EXPECTED_HEAD_SHA = "b9cdc9e797b36709ba4c17c23a4c6585670254e3"
EXPECTED_ARTIFACT_NAME = f"clue-jls2-public-validation-v1-{EXPECTED_RUN_ID}"
...
def verify_complete_checksum_manifest(root: Path) -> dict[Path, str]:
    ...
    if symlinks:
        raise ValueError(f"evidence contains a symlink: {symlinks[0].relative_to(root)}")
    ...
    if unlisted:
        raise ValueError(f"evidence contains an unlisted file: {unlisted[0].as_posix()}")
...
def verify_publication_bundle(publication: Path) -> dict[str, Any]:
    ...
    if bundle.get("name") != "clue-jls2-public-validation-first-score-bundle-v1":
...
def import_evidence(
    *,
    ...
    try:
        ...
        receipt = {
            "schema_version": 1,
            "name": "clue-jls2-public-validation-first-score-import-v1",
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=REPOSITORY / "runs" / "clue-jls2-public-validation-v1",
    )
    parser.add_argument(
        "--import-receipt",
        type=Path,
        default=REPOSITORY / "runs" / "clue-jls2-public-validation-v1-import.json",
    )
    parser.add_argument(
        "--expected-gates",
        type=Path,
    ...
    try:
        ...
        raise SystemExit(f"CLUE-LDS first-score import refused: {error}") from error
    print(receipt)
    return 0
