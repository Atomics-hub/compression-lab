#!/usr/bin/env python3
"""Verify the exact final CLUE JLS2 championship-screen v1 readiness lock."""
...
DEFAULT_LOCK = REPOSITORY / "config" / "clue-jls2-championship-screen-v1-lock.json"
...
def verify_lock(
    ...
    *,
    ...
    if not lock_path.is_file():
        raise ValueError("final CLUE JLS2 championship readiness lock is missing")
    lock = json.loads(lock_path.read_text(encoding="utf-8"))
    if lock.get("name") != LOCK_NAME:
        raise ValueError("unexpected CLUE JLS2 championship readiness lock identity")
    readiness = str(lock["readiness_commit"])
    head = git("rev-parse", "HEAD").stdout.decode().strip()
    status = (
        git("status", "--porcelain", "--untracked-files=no").stdout.decode().strip()
    )
    if require_clean and status:
        raise ValueError("championship screen requires a clean tracked tree")
    ...
    if authorization["expected_item_ids"] != EXPECTED_ITEM_IDS:
