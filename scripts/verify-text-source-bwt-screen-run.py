#!/usr/bin/env python3
"""Verify every retained BWT screen receipt and recompute its decision."""
...
RUNNER = load_script(
    "bwt_screen_runner_for_verifier",
...
def validate_preflight(rows: object, config: dict[str, Any]) -> None:
    if (
        ...
        raise ValueError("BWT preflight roster differs")
    for row in rows:
        if (
            ...
            or any(character not in "0123456789abcdef" for character in row["artifact_sha256"])
            or row.get("exact_roundtrip") is not True
        ):
            raise ValueError("BWT preflight evidence differs")
...
def verify(
    *,
    ...
    if (
        result.get("name") != "text-source-bwt-kanzi-decomposition-screen-result-v1"
        ...
        or any(character not in "0123456789abcdef" for character in bindings["repository_commit"])
    ):
        raise ValueError("BWT result identity or binding differs")
    ...
    if len(expected_paths) != 32:
        raise ValueError("BWT expected receipt roster is not exactly 32 trials")
    ...
    if result.get("summary") != expected_summary:
        raise ValueError("BWT decision does not reconstruct from receipts")
    if result_raw != RUNNER.json_bytes(result):
        raise ValueError("BWT result is not canonical")
    return {
        "verified": True,
        ...
        "shared_signal_variants": expected_summary["shared_signal_variants"],
...
def main() -> int:
    ...
    parser.add_argument("--structural-result", type=Path, default=RUNNER.DEFAULT_STRUCTURAL_RESULT)
    parser.add_argument("--predictor-result", type=Path, default=RUNNER.DEFAULT_PREDICTOR_RESULT)
    parser.add_argument("--long-range-result", type=Path, default=RUNNER.DEFAULT_LONG_RANGE_RESULT)
    parser.add_argument("--record-result", type=Path, default=RUNNER.DEFAULT_RECORD_RESULT)
    ...
    try:
        ...
        raise SystemExit(f"BWT run verification failed: {error}") from error
