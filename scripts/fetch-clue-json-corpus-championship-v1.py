#!/usr/bin/env python3
"""Acquire the two sealed CLUE-LDS championship-screen NDJSON ranges exactly once.

This third acquisition path mirrors the frozen v2 fetcher discipline. It refuses
any range that overlaps a range already declared by any tracked corpus
configuration - every consumed v1 and v2 range AND the two championship ranges,
which are mutually refused for any future acquisition - and refuses to open the
public-validation split without a valid final championship readiness lock over a
clean worktree.
"""
...
DEFAULT_CONFIG = REPOSITORY / "config" / "clue-json-log-corpus-championship-v1.json"
DEFAULT_VALIDATION_LOCK = (
    REPOSITORY / "config" / "clue-jls2-championship-screen-v1-lock.json"
)
LOCK_VERIFIER = (
...
# or reserved: v1 (development + validation-a/b), v2 (validation-c/d), and the
# championship ranges themselves. A championship range that overlaps any of
# these with a different id is refused.
DECLARED_RANGE_CONFIGS = (
    REPOSITORY / "config" / "clue-json-log-corpus-v1.json",
    REPOSITORY / "config" / "clue-json-log-corpus-v2.json",
    REPOSITORY / "config" / "clue-json-log-corpus-championship-v1.json",
)
...
def verify_validation_lock(lock_path: Path) -> dict[str, Any]:
    ...
    if specification is None or specification.loader is None:
        raise ValueError("unable to load the CLUE JLS2 championship readiness-lock verifier")
...
def assert_ranges_available(
    ...
    entry in the championship corpus config); it may not overlap any declaration
    with a different id in any tracked corpus configuration.
    """
...
def download(url: str, destination: Path) -> None:
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "compression-lab-corpus-fetch/3"},
    )
...
def build(
    ...
    *,
    ...
    if normalized_split == "public_validation" and not allow_public_validation:
        raise ValueError(
            "refusing to acquire CLUE championship public validation without "
    ...
    if normalized_split == "public_validation":
        try:
            ...
            raise ValueError(
                "refusing CLUE championship public validation without a valid final "
                f"readiness lock over a clean worktree: {error}"
            ) from error
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=REPOSITORY / "corpora" / "clue-json-log-championship-v1",
    )
    parser.add_argument(
        "--cache",
        type=Path,
        default=REPOSITORY / "corpora" / "_download-cache" / "clue-championship",
    )
