...
def main() -> int:
    ...
    for item in manifest["items"]:
        ...
        transformed_bytes = len(plane_transform(source))
        candidates = []
        for level in levels:
            start = time.perf_counter_ns()
            frame = plane_compress(source, level=level)
            compression_ns = time.perf_counter_ns() - start
            start = time.perf_counter_ns()
            restored = plane_decompress(frame)
            decompression_ns = time.perf_counter_ns() - start
            if restored != source:
                raise RuntimeError(f"DMP1 round trip failed: {item['id']}")
            candidates.append(
                {
                    "level": level,
    ...
    result = {
        "schema_version": 1,
        "name": "dmp1-row-major-numeric-bitplane-probe-v1",
