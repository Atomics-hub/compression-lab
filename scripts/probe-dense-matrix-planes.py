...
def main() -> int:
    ...
    for item in manifest["items"]:
        ...
        transformed_bytes = len(matrix_transform(source))
        candidates = []
        for level in levels:
            start = time.perf_counter_ns()
            frame = matrix_compress(source, level=level)
            compression_ns = time.perf_counter_ns() - start
            start = time.perf_counter_ns()
            restored = matrix_decompress(frame)
            decompression_ns = time.perf_counter_ns() - start
            if restored != source:
                raise RuntimeError(f"DMI1 round trip failed: {item['id']}")
            candidates.append(
                {
                    "level": level,
        ...
        gain = (baseline_bytes - best["complete_bytes"]) / baseline_bytes * 100
        rows.append(
            {
                "id": item["id"],
                "family": item["family"],
    ...
    result = {
        "schema_version": 1,
        "name": "dmi1-column-major-token-plane-probe-v1",
