#!/usr/bin/env python3
"""Evaluate the frozen first-score CLUE-LDS JLS2 validation gate."""
...
def evaluate(
    ...
    if not receipt.get("first_score") or not receipt.get("completed"):
        raise ValueError("JLS2 first-score receipt is incomplete")
    ...
    family_by_id = {
        row["id"]: row["family"] for row in validation["expected_items"]
    }
    ...
    for item_id in expected_ids:
        ...
        if set(by_codec) != set(expected_codecs):
            raise ValueError(f"family codec roster differs from frozen contract: {item_id}")
    ...
    gate_results = {
        ...
        "compression_memory": int(candidate_summary["compression_peak_rss_bytes"])
        <= requirements["maximum_cold_compression_peak_rss_mib"] * MIB,
        "decompression_memory": int(decode["peak_rss_bytes"])
        <= requirements["maximum_cold_decompression_peak_rss_mib"] * MIB,
        ...
        comparison_rows.append(
            {
                ...
                "compression_peak_rss_bytes": int(row["compression_peak_rss_bytes"]),
                "decompression_peak_rss_bytes": (
                    int(decode["peak_rss_bytes"])
                    if is_candidate
    ...
    return {
        "schema_version": 1,
        "name": "clue-jls2-public-validation-first-score-v1",
        ...
        "aggregate": {
            ...
            "standalone_decompression_mbps": float(
                decode["median_aggregate_mbps"]
            ),
            "compression_peak_rss_bytes": int(
                candidate_summary["compression_peak_rss_bytes"]
            ),
            "standalone_decompression_peak_rss_bytes": int(
                decode["peak_rss_bytes"]
            ),
        },
