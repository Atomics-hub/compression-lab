#!/usr/bin/env python3
"""Publish the immutable CLUE-LDS JLS2 first-score evidence and chart."""
...
def render_svg(decision: dict[str, Any]) -> str:
    available = [
        row for row in decision["comparison_rows"] if row["compressed_bytes"] is not None
    ]
    if not available:
    ...
    elements = [
        ...
        '<title id="title">Atompress JLS2 CLUE-LDS compressed-byte comparison</title>',
        ...
        'font-size="22" font-weight="700">Atompress (JLS2) — complete compressed bytes</text>',
        '<text x="28" y="57" fill="#94a3b8" font-family="system-ui,sans-serif" '
        'font-size="13">Lower is better · exact-byte archive accounting · first frozen score</text>',
    ]
...
def render_report(decision: dict[str, Any]) -> str:
    ...
    for row in decision["comparison_rows"]:
        ...
        smaller_text = "candidate" if row["role"] == "candidate" else (
            "yes" if smaller is True else "no" if smaller is False else "not measured"
        )
        exact = row["exact_roundtrip"]
        exact_text = "yes" if exact is True else "no" if exact is False else "unavailable"
    ...
    for name, gate_passed in decision["gate_results"].items():
        lines.append(f"| {name.replace('_', ' ')} | {'pass' if gate_passed else 'fail'} |")
    lines.extend(
        [
            "",
            "## Evidence boundary",
...
def publish(
    *,
    ...
    if decision.get("name") != "clue-jls2-public-validation-first-score-v1":
    ...
    bundle = {
        "schema_version": 1,
        "name": "clue-jls2-public-validation-first-score-bundle-v1",
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"CLUE-LDS publication refused: {error}") from error
    print(bundle)
    return 0
